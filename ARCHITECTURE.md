...
**Filters and scope apply before limit, never to page after it.** That is whole point: filtering fetched tail answers "among newest N rows", which reads identical to "in the log" and is not. `pageLogs` in `@omni/control` takes caller filters and verified `Scope` as *separate arguments* — filters can only narrow, scope comes from session, and scope's key is written after caller's `apiKeyId` so client sending one cannot widen. `readsNothing` checked **before** `scopeKey`, same trap as `recentLogs`: `scopeKey` collapses `all` and `none` to same `undefined`. Cursor parsed **before** that early return, so client's malformed cursor is `BAD_REQUEST` rather than empty page reading as end of its history. Wire cursor is versioned base64url JSON (`{"v":1,"at":…,"id":"…"}`); every decode failure is `BAD_REQUEST` and none falls back to newest page. Indexes carrying it: `(at, id)`, `(api_key_id, at, id)`, `(credential_id, at, id)` — migration 015 replaced shorter prefixes rather than adding beside them, so no redundant index costs write per row. Migration 016 added one per exact filter — `resolved_model`, `requested_model`, `error_code`, `resolved_provider`, each leading with filtered column then keyset pair — because unindexed exact filter scanned whole table whenever it matched nothing, 15ms of synchronous `bun:sqlite` per miss. `state` excluded: `idx_request_logs_pending` cover selective half and `done` is every row. `model` is only OR clause, so it plan as MULTI-INDEX OR: two seeks unioned, union sorted in temp B-tree because it arrive in two orders and neither is `at DESC`. **Limit therefore stop being early exit for that plan**, so it pay for how *common* value is where ordered scan pay for how *rare* — both grow with table (1M rows: scan 149ms on miss, union 41ms when 55% of rows match). Union chosen because its worst case some 3.6x smaller and is case operator *asked for*, where scan's worst case is one they make by typing. Levers when it hurt: time bound, or `resolved:`/`requested:` (one seek, no union). Upgrade past that is covering index on `(at DESC, id DESC)` including both model columns. **Trap: `ANALYZE` cost union's sort against ordered scan and pick scan** — full table again, measured at 5k rows — so nothing under `packages/store/src` may run it; plan test run on empty database and cannot see it, source-reading test in `migrations.test.ts` is guard.

Every filter is `=` save one: `model` matches requested **or** resolved model, `AND`ed with rest like any other. It exists because two columns hold different vocabularies — alias like `opus` only ever requested, `claude-opus-5` it routes to only ever resolved — so operator naming model cannot know which side their spelling is on, and asking one column reads as "no such traffic" rather than "wrong column". Narrow `requestedModel`/`resolvedModel` stay for question that really is about one side. Console's box and `omni logs --model` both send `model`; box **debounces**, since every prefix of exact name matches nothing and matches it by full scan. `toQuery` copies filters one key at time, so key it forgets is filter wire accepts and nothing applies — `.strict()` cannot see it, parameter *is* known — pinned as whole set by `logPage.test.ts`.
