...
- Cached-token field names differ by surface: `input_tokens_details.cached_tokens` on the Responses API, `prompt_tokens_details.cached_tokens` on chat-completions. Both decoders read their own name first and fall back to the other, because an OpenAI-compatible endpoint behind either target may use either.
- The four token classes are disjoint and each is priced once, in `apps/gateway/src/dispatch/price.ts`. Cache writes cost *more* than fresh input, not less: Anthropic bills a 5m write at 1.25x base input and a 1h write at 2x, so `Usage` carries the per-TTL split Anthropic reports in `cache_creation` and a target prices each separately. A target that names no write price falls back to those multipliers rather than to zero — a provider that bills no premium says so with an explicit zero, which the catalog carries for OpenAI and Kimi.
