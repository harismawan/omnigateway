...
export type ConsoleDeps = {
  /** Null when the file does not exist, so an absent log is not an error. */
...
async function readSource(
  ...
  if (source.kind === "file") return deps.readFile(source.path) ?? "";
 ...
 *
 * The tail is taken *after* filtering, so a page asked to show errors is filled
 * with errors rather than with whatever happened to fall in the last N lines.
 */
export async function readConsole(
  ...
  const text = await readSource(deps, source, query.lines);
  const lines = text
    .split("\n")
