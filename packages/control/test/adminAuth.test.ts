...
test("verifies existing legacy hashes from @node-rs/argon2", async () => {
  const store = await memoryStore();
  const auth = createAdminAuth(store, opts);
  // Hash generated by @node-rs/argon2 with OWASP baseline (m=19456, t=2, p=1)
  await store.config.setAdminPasswordHash(
    "$argon2id$v=19$m=19456,t=2,p=1$l3SA90jOgwe6OXTFGCdu6Q$MlCWIoHV+Wa1DwZZkoJpnR4syDmJlkdiJdeDzjzHL9o",
  );
  expect(await auth.login("testpassword123")).not.toBeNull();
  expect(await auth.login("wrongpassword")).toBeNull();
});
